...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 23-11-2018 a las 01:36:01
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `buscarComentario` (IN `id` INT(11))  BEGIN
  select calificacion_x_producto.id,comentario,calificacion,idProducto,users.name from calificacion_x_producto
inner join users on email = idUsuario
 where calificacion_x_producto.id = id;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `buscarProductoxID` (IN `pID` INT)  BEGIN

SELECT p.idProducto, p.nombre, p.descripcion, p.precio, p.imagen, p.precio, p.stock, p.estado,p.promedio, pc.Categoria_idCategoria, c.nombre as Cnombre
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `busqueda_producto` (IN `frase` VARCHAR(45), IN `IDcategoria` INT)  BEGIN
...
SELECT * FROM producto WHERE nombre LIKE CONCAT('%', frase, '%') and estado = 1;
 ELSE
  ...
  where Producto.estado = 1 and Producto.nombre LIKE CONCAT('%', frase, '%');
  END IF;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getCategorias` ()  BEGIN
  select idCategoria, nombre,condicion from Categoria where condicion = 1;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `getComentarios` (IN `producto` INT(11))  BEGIN
  select calificacion_x_producto.id,comentario,calificacion,idProducto,users.name from calificacion_x_producto
inner join users on email = idUsuario
 where idProducto = producto LIMIT 15;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `getProductos` ()  BEGIN
  select * from Producto
  order by idProducto desc LIMIT 20;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `getProductosHabilitados` ()  BEGIN
select * from Producto
  where estado = 1
  order by idProducto desc LIMIT 20;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `getRespuestas` (IN `comentario` INT(11))  BEGIN
select idRespuesta,respuesta,idCalificacion,users.name from respuestas
inner join users on email = idUsuario
 where idCalificacion = comentario LIMIT 15;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertarRespuesta` (IN `calificacion` INT(11), IN `texto` VARCHAR(300), IN `usuario` VARCHAR(255))  BEGIN
insert into respuestas (idCalificacion,respuesta,idUsuario)
    values (calificacion, texto,usuario);
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `nuevoComentario` (IN `texto` VARCHAR(300), IN `puntuacion` INT(11), IN `producto` INT(11), IN `usuario` VARCHAR(255))  BEGIN
  insert into calificacion_x_producto (comentario,calificacion,idProducto,idUsuario)
    values (texto, puntuacion,producto,usuario);
    update producto set calificaciones = calificaciones + 1,
    promedio = (promedio + puntuacion)/calificaciones
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `productos_x_categoria` (IN `IDcategoria` INT)  BEGIN
  select idProducto,nombre,descripcion, imagen, precio,stock,promedio from Producto
  inner join categoria_x_producto on idProducto = Producto_idProducto and Categoria_idCategoria = IDcategoria
...
--
-- Estructura de tabla para la tabla `calificacion_x_producto`
--

CREATE TABLE IF NOT EXISTS `calificacion_x_producto` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `comentario` varchar(300) COLLATE utf8_bin DEFAULT NULL,
  `calificacion` int(11) NOT NULL DEFAULT '0',
  `idProducto` int(11) NOT NULL,
  `idUsuario` varchar(255) CHARACTER SET utf8 NOT NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `respuestas` (
  ...
  `respuesta` varchar(300) COLLATE utf8_bin NOT NULL,
  `idUsuario` varchar(255) CHARACTER SET utf8 NOT NULL,
  PRIMARY KEY (`idRespuesta`),
  KEY `fk_respuestas_calificacion_idx` (`idCalificacion`),
...
--
-- Filtros para la tabla `calificacion_x_producto`
--
ALTER TABLE `calificacion_x_producto`
  ADD CONSTRAINT `fk_producto` FOREIGN KEY (`idProducto`) REFERENCES `producto` (`idProducto`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `fk_usuario` FOREIGN KEY (`idUsuario`) REFERENCES `users` (`email`) ON DELETE NO ACTION ON UPDATE NO ACTION;
...
ALTER TABLE `respuestas`
  ADD CONSTRAINT `fk_respuesta_usuario` FOREIGN KEY (`idUsuario`) REFERENCES `users` (`email`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `fk_respuestas_calificacion` FOREIGN KEY (`idCalificacion`) REFERENCES `calificacion_x_producto` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;
